...
short numberOfVacationDays(stDate start, stDate end) {

    short vactionDays = 0;

    while (isDate1LessThenDate2(start, end)) {
...
int main()
{
    ...
    cout << endl;
    cout << "Vacation End: \n";
    stDate endVactionDate = readDate();


    cout << "\nVaction from:" << dayShortName(dayOfWeekOrder(startVactionDate)) << " , " <<
        startVactionDate.day << "/" << startVactionDate.month << "/" << startVactionDate.year << "/";
    cout << "\nVaction End:" << dayShortName(dayOfWeekOrder(endVactionDate)) << " , " <<
        endVactionDate.day << "/" << endVactionDate.month << "/" << endVactionDate.year << "/";

    cout << "\nActucal Vaction Days is :" << numberOfVacationDays(startVactionDate, endVactionDate);
