...
void updateClient(stClient client, vector<stClient>& clients) {
...
bool updateClientByAccountNumber(string accountNumber , vector<stClient> clients , stClient client) {
    if (findClientByAccountNumber(accountNumber, clients, client)) {
        printClient(client);

        char choice;

        cout << "\nDo you want to updated this client? [Y/N]";
        cin >> choice;

        if (tolower(choice) == 'y') {
            updateClient(client, clients);
            saveClientDataIntoFile("MyFile.txt", clients);
            printf("Client with AccountNumber:%s is updated Successfully.", accountNumber);
        }
...
int main()
{
    ...
    updateClientByAccountNumber(accountNumber, clients, client);
